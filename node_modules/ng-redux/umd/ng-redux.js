...
function Connector(store) {
  return function (mapStateToTarget, mapDispatchToTarget) {
    ...
    return function (target) {
      ...
      invariant_1(isFunction_1(target) || isObject_1(target), 'The target parameter passed to connect must be a Function or a object.');
...
function getStateSlice(state, mapStateToScope) {
  ...
  if (shouldReturnObject) {
    invariant_1(isPlainObject_1(slice), '`mapStateToScope` must return an object. Instead received %s.', slice);
  } else {
    invariant_1(isPlainObject_1(slice) || isFunction_1(slice), '`mapStateToScope` must return an object or a function. Instead received %s.', slice);
  }
...
function digestMiddleware($rootScope, debounceConfig) {
  ...
  if (debounceConfig && debounceConfig.wait && debounceConfig.wait > 0) {
    debouncedFunction = debounce_1(debouncedFunction, debounceConfig.wait, { maxWait: debounceConfig.maxWait });
  }
...
var _arrayIncludes$2 = arrayIncludes;
...
var typeIs = curry_1(function (type, val) {
  return (typeof val === 'undefined' ? 'undefined' : _typeof(val)) === type;
});
var isObject$4 = typeIs('object');
...
function ngReduxProvider() {
  ...
  this.createStoreWith = function (reducer, middlewares, storeEnhancers, initialState) {
    invariant_1(isFunction_1(reducer) || isObject$4(reducer), 'The reducer parameter passed to createStoreWith must be a Function or an Object. Instead received %s.', typeof reducer === 'undefined' ? 'undefined' : _typeof(reducer));

    invariant_1(!storeEnhancers || isArray$3(storeEnhancers), 'The storeEnhancers parameter passed to createStoreWith must be an Array. Instead received %s.', typeof storeEnhancers === 'undefined' ? 'undefined' : _typeof(storeEnhancers));

    _reducer = reducer;
    _reducerIsObject = isObject$4(reducer);
    _storeEnhancers = storeEnhancers || [];
  ...
  this.$get = function ($injector) {
    ...
    var resolvedMiddleware = map_1(_middlewares, resolveMiddleware);
    ...
    var resolvedStoreEnhancer = map_1(_storeEnhancers, resolveStoreEnhancer);
    ...
    if (_reducerIsObject) {
      ...
      var resolveReducerKey = function resolveReducerKey(result, key) {
        return assign$4({}, result, _defineProperty$1({}, key, getReducerKey(key)));
      };
